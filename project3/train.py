...
def train(model_G, model_D, optimizer, train_set, test_set, num_workers=8, num_epochs=10, batch_size=64, loss_funcs={"GAN":nn.MSELoss, "Cycle": nn.L1Loss, "Identity": nn.L1Loss}):
    ...
    for epoch in tqdm(range(num_epochs), unit='epoch'):
        ...
        if (epoch+1) % 10 == 0:
            print(f"hello{epoch}")
            # weights_path = os.path.join(wandb.run.dir,f"G_a2b{epoch+1:03d}.pth")
            # torch.save(G_a2b.state_dict(),weights_path)
            # wandb.save(weights_path)
            # weights_path = os.path.join(wandb.run.dir,f"G_b2a{epoch+1:03d}.pth")
            # torch.save(G_b2a.state_dict(),weights_path)
            # wandb.save(weights_path)
            # weights_path = os.path.join(wandb.run.dir,f"D_a{epoch+1:03d}.pth")
            # torch.save(D_a.state_dict(),weights_path)
            # wandb.save(weights_path)
            # weights_path = os.path.join(wandb.run.dir,f"D_b{epoch+1:03d}.pth")
